...
class UserGroupPermissionsController extends UserMgmtAppController {
	...
	var $uses = array('Usermgmt.UserGroupPermission','Usermgmt.UserGroup','Usermgmt.User');
	...
	private function __deleteCache() {
		...
		foreach (new RecursiveIteratorIterator($iterator, RecursiveIteratorIterator::CHILD_FIRST) as $file) {
         ...
         /**
     * @method tab_access_permission
     * @uses It's use to get user access data from current logged in user id.
     * @return array
     */
    public function tab_access_permission() {
        $this->layout = 'ajax';
        $user_id = $this->UserAuth->getUserId();
      $user=$this->User->findById($user_id);
       $group_id=$user['User']['user_group_id'];
        $conditions[] = array('OR' => array('UserGroupPermission.user_group_id =' => $group_id));
  
 //  echo $group_id;
  // exit;
    $permission = $this->UserGroupPermission->find('all', array('conditions'=>$conditions));
 // pr($permission);
   //   exit;
      //$permission = $this->UserAccess->findByuser_id($user_id);
        return $permission;
    }
}